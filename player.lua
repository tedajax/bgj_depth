...
function create_player(x, y)
    ...
    self.original_position = { x = x, y = y }
    ...
    self.reset = function(self)
        self.position = { x = self.original_position.x, y = self.original_position.y }
        self.rotation = 0
        self.health = self.max_health
        self.state = PlayerState.cAlive
        self.hit_timer = 0
...
function create_player_controller(player)
    ...
    self.update = function(self, dt)
        if self.player.state == PlayerState.cAlive then
            ...
            if self.player.position.y > 120 then
                self.player.position.y = 120
            end
        elseif self.player.state == PlayerState.cDead then
            if self.explosion_timer > 0 then
                ...
                Game.explosion_manager:add(self.player.position.x + math.random(-48, 48),
                                           self.player.position.y + math.random(-48, 48),
                                           1)
        ...
        if Input:get_button("fire") then
            ...
            if self.fire_timer <= 0 then
                local bvx = Input:get_axis("horizontal") * self.speed + Game.move_speed
